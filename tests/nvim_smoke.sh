...
# cursor and stop cleanly. diktat is stubbed (printf); no model needed.
set -e
cd "$(dirname "$0")/.."
...
nvim --headless -u NONE \
    +"set rtp+=$(pwd)" \
    +"lua require('geist-diktat').setup({ cmd = [[printf 'hello world\n']] })" \
    +"lua require('geist-diktat').start()" \
    ...
    +"lua require('geist-diktat').stop()" \
    +"lua assert(not require('geist-diktat').is_active(), 'job not stopped')" \
    +"qa!" 2>/dev/null
